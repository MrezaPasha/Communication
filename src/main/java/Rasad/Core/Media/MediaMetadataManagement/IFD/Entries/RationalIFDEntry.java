...
*/
public class RationalIFDEntry implements IFDEntry
{
...
//ORIGINAL LINE: private void setTag(ushort value)
	...
	*/
	private Rational Value = new Rational();
	public final Rational getValue()
	{
		return Value;
	}
	private void setValue(Rational value)
	{
		Value = value.clone();
	}
...
///#region Constructors
	...
	*/
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public RationalIFDEntry(ushort tag, Rational value)
	public RationalIFDEntry(short tag, Rational value)
	{
		setTag(tag);
...
//ORIGINAL LINE: type = (ushort) IFDEntryType.Rational;
		...
		data.add(ByteVector.FromUInt(getValue().getNumerator(), is_bigendian));
		data.add(ByteVector.FromUInt(getValue().getDenominator(), is_bigendian));
