...
*/
public class SRationalIFDEntry implements IFDEntry
{
...
//ORIGINAL LINE: private void setTag(ushort value)
	...
	*/
	private SRational Value = new SRational();
	public final SRational getValue()
	{
		return Value;
	}
	private void setValue(SRational value)
	{
		Value = value.clone();
	}
...
///#region Constructors
	...
	*/
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public SRationalIFDEntry(ushort tag, SRational value)
	public SRationalIFDEntry(short tag, SRational value)
	{
		setTag(tag);
...
//ORIGINAL LINE: public ByteVector Render(bool is_bigendian, uint offset, out ushort type, out uint count)
	...
	{
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: type = (ushort) IFDEntryType.SRational;
		type.argValue = (short) IFDEntryType.SRational.getValue();
		count.argValue = 1;

		ByteVector data = new ByteVector();
		data.add(ByteVector.FromInt(getValue().getNumerator(), is_bigendian));
		data.add(ByteVector.FromInt(getValue().getDenominator(), is_bigendian));
