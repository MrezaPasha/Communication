...
*/
public class IsoChunkLargeOffsetBox extends FullBox
{
...
//ORIGINAL LINE: private ulong [] offsets;
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	public IsoChunkLargeOffsetBox(BoxHeader header, Rasad.Core.Media.MediaMetadataManagement.File file, IsoHandlerBox handler)
	{
		super(header.clone(), file, handler);
		ByteVector box_data = file.ReadBlock(getDataSize());

//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: offsets = new ulong [(int) box_data.Mid(0, 4).ToUInt()];
		offsets = new long [(int) box_data.Mid(0, 4).ToUInt()];
...
//ORIGINAL LINE: ByteVector output = ByteVector.FromUInt((uint) offsets.Length);
	...
	*/
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public ulong [] getOffsets()
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	public final ByteVector Render(long sizeDifference, long after)
	{
		...
		{
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: if (offsets [i] >= (ulong) after)
			if (offsets [i] >= (long) after)
			{
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: offsets [i] = (ulong)((long) offsets [i] + sizeDifference);
				offsets [i] = (long)((long) offsets [i] + sizeDifference);
			}
		}
