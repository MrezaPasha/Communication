...
*/
public class ZipAESStream extends CryptoStream
{

	/** 
	 Constructor
	 
	 @param stream The stream on which to perform the cryptographic transformation.
	 @param transform Instance of ZipAESTransform
...
//ORIGINAL LINE: _slideBuffer = new byte[1024];
		...
		{
			throw new RuntimeException("ZipAESStream only for read");
		}
	}
...
//ORIGINAL LINE: private byte[] _slideBuffer;
	...
	*/
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public override int Read(byte[] outBuffer, int offset, int count)
	@Override
	public int Read(byte[] outBuffer, int offset, int count)
	{
		...
		while (nBytes < count)
		{
			...
			else
			{
				...
				{
					throw new RuntimeException("Internal error missed auth code"); // Coding bug
				}
				// Final block done. Check Auth code.
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: byte[] calcAuthCode = _transform.GetAuthCode();
				byte[] calcAuthCode = _transform.GetAuthCode();
				for (int i = 0; i < AUTH_CODE_LENGTH; i++)
				{
					if (calcAuthCode[i] != _slideBuffer[_slideBufStartPos + i])
					{
						throw new RuntimeException("AES Authentication Code does not match. This is a super-CRC check on the data in the file after compression and encryption. \r\n" + "The file may be damaged.");
					}
				}
	...
	/** 
	 ...
	 @param offset The byte offset in buffer at which to begin copying bytes to the current stream. 
