...
//ORIGINAL LINE: [KnownType(typeof(ServiceType))][DataContract][ProtoContract] public class TRequestUninstallService : TMessageBase
public class TRequestUninstallService extends TMessageBase
{
	private TRequestUninstallService()
	{
	}
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public TRequestUninstallService(byte serverID, string serviceName)
	public TRequestUninstallService(byte serverID, String serviceName)
	{
		super(serverID);
...
//C# TO JAVA CONVERTER WARNING: Unsigned integer types have no direct equivalent in Java:
//ORIGINAL LINE: public TRequestUninstallService(byte serverID, Nullable<Guid> pluginWindowsServiceKey)
	public TRequestUninstallService(byte serverID, UUID pluginWindowsServiceKey)
	{
		super(serverID);
...
//ORIGINAL LINE: [DataMember][ProtoMember(3)] public Nullable<Guid> PluginWindowsServiceKey {get;set;}
	...
	public String toString()
	{
		...
		{
			return String.format("RequestUninstallService - Server ID : %1$s - Plugin Windows Service Key : %2$s", getServerID(), getPluginWindowsServiceKey());
		}
		else
		{
			return String.format("RequestUninstallService - Server ID : %1$s - Service Name : %2$s", getServerID(), getServiceName());
		}
	}
