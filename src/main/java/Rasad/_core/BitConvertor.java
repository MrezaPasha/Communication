...
import Rasad.Core.Media.MediaMetadataManagement.Asf.DataType;
...
public class BitConvertor {
    ...
    public static int sum (ArrayList<byte[]> bytes) throws Exception
    {
        int sum =0;
        for (byte[] b:bytes) {
            sum += BitConvertor.toInt32(b,b.length);
        }
        return sum;
    }
